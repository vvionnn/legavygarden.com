...
def index():
    ...
    try:
        role = (me["role"] or "youth").lower()
    except Exception:
        role = "youth"

    stories = db_helper.get_stories_for_pov(role, uid)
    stories = sorted(stories, key=lambda s: s.get("id", 0), reverse=True)
...
# =====================================
        if save_as == "draft":
            ...
            DEFAULT_IMAGE = "default_logo.png"
            ...
            if title.strip() and (contains_bad_word(title) or looks_like_gibberish(title)):
                return render_template("story/create.html", errors={"title": "Content not allowed."}, form_data=request.form, draft_id=draft_id)

            if content.strip() and (contains_bad_word(content) or looks_like_gibberish(content)):
                return render_template("story/create.html", errors={"content": "Content not allowed."}, form_data=request.form, draft_id=draft_id)
        ...
        role_visibility = (me["role"] or "Youth").lower()
        ...
        # ✅ Log water earned (shows in Water History)
        try:
            db_helper.log_garden_history(
                user_id=session.get("user_id", 1),
                category="water",
                title=f'Earned water from story "{title}" (+{water} water)',
                amount=int(water)
            )
        except Exception:
            pass
...
def view_story(story_id):
    ...
    me = db_helper.get_user_by_id(uid)
    my_role = (me["role"] or "youth").lower()

    author_role = (story.get("role_visibility") or "").lower()
    author_id = story.get("user_id")

    if author_id != uid and author_role == my_role:
